...
for iter = 1:3000
    ...
    %提示：x_near已经在树T里
    x_near_Idx = 1;
    x_near_minDis = sqrt((x_rand(1)-T.v(x_near_Idx).x)^2 + (x_rand(2)-T.v(x_near_Idx).y)^2 );
    for i=1:count
        dis_temp = sqrt((x_rand(1)-T.v(i).x)^2 + (x_rand(2)-T.v(i).y)^2 );
        if(dis_temp<x_near_minDis)
            x_near_minDis = dis_temp;
            x_near_Idx = i;
        end
    end
    ...
    %Step 3: 扩展得到x_new节点
    %提示：注意使用扩展步长Delta
    x_new = x_near + (x_rand-x_near)./x_near_minDis.*Delta;
